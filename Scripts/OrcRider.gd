...
## Orc Rider — Fast mounted enemy with charge attack

@export var move_speed: float = 90.0
@export var max_health: int = 6
@export var contact_damage: int = 2
@export var attack_range: float = 30.0
@export var charge_range: float = 60.0
...
var is_charging: bool = false; var charge_target_pos: Vector2
...
func _physics_process(_delta: float) -> void:
	...
	if is_charging:
		var to_charge := charge_target_pos - global_position
		if to_charge.length() > 5.0:
			velocity = to_charge.normalized() * charge_speed
		else:
			velocity = Vector2.ZERO; is_charging = false
...
func _on_attack_timer() -> void:
	...
	match attack_cycle % 4:
		0:
			if dist <= attack_range: _play_action(&"attack01"); swing_damage = attack01_damage
		1:
			if dist <= attack_range: _play_action(&"attack02"); swing_damage = attack02_damage
		2: # Charge attack — lunge toward target
			if dist <= charge_range:
				_play_action(&"attack03"); swing_damage = attack03_damage
				charge_target_pos = target.global_position; is_charging = true
		3:
			_play_action(&"block"); swing_damage = 0
	attack_cycle += 1
...
func _on_animation_finished() -> void:
	if animated_sprite.animation == &"death":
		ExpGemScript.drop_gems(self, 1, 2)  # Medium tier, 2 gems
		queue_free(); return
	if animated_sprite.animation == current_action_animation:
		current_action_animation = &""; is_charging = false
		swing_damage = 0
		swing_hit_targets.clear()
...
func _on_frame_changed() -> void:
	...
	elif anim == &"attack03" and f == 3: CombatSound.play_random_slash(self)
