...
func _on_attack_timer() -> void:
	...
	match attack_cycle % 4:
		0:
			if dist <= attack_range: _play_action(&"attack01"); _dmg_target(attack01_damage)
		1:
			if dist <= attack_range: _play_action(&"attack02"); _dmg_target(attack02_damage)
		2: # Charge attack — lunge toward target
			if dist <= charge_range:
				_play_action(&"attack03"); _dmg_target(attack03_damage)
...
func _on_animation_finished() -> void:
	if animated_sprite.animation == &"death":
		...
		queue_free(); return
	if animated_sprite.animation == current_action_animation: current_action_animation = &""; is_charging = false
